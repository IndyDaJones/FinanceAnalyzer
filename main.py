...
def update_signal(signal_type, signal_data):
    try:
        data = MySQL()
        data.connect()
        for dates in signal_data:
            data.updateSignal(dates, signal_type)
...
def main():
    try:
        ...
        for stock in symbols:
            ...
            ticker1 = pd.read_sql(sql_query,connection)
            
            macd, exp3 = claculate_macd(ticker1)
            buy_signal, sell_signal, macd_signal = implement_macd_strategy(ticker1, macd, exp3)
            
            update_signal("BUY", buy_signal)
            update_signal("SELL", sell_signal)
